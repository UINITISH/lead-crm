...
export async function addTicketEvent(ticketId, { event_type, from_status = null, to_status = null, note = null, actor = 'system' }) {
  await query(
...
export async function createTicket({ subject, description = null, department = 'general', priority = 'medium',
  ...
  const res = await query(
    `INSERT INTO tickets (subject, description, department, priority, lead_id, requester, assignee, created_by)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
    [subject.trim(), description, department, priority, lead_id, requester, assignee, created_by],
  );
...
export async function updateTicket(id, { subject, description, department, priority, status, assignee, requester, note, actor } = {}) {
  const current = await one(`SELECT * FROM tickets WHERE id = $1`, [id]);
  ...
  const res = await query(
    `UPDATE tickets SET
        subject = $2, description = $3, department = $4, priority = $5,
        status = $6, assignee = $7, requester = $8, resolved_at = $9
  ...
  if (status && status !== current.status) {
    await addTicketEvent(id, { event_type: 'status_change', from_status: current.status, to_status: status, note, actor: actor || 'admin' });
  } else if (note) {
    await addTicketEvent(id, { event_type: 'note', note, actor: actor || 'admin' });
  } else if (assignee !== undefined && assignee !== current.assignee) {
    await addTicketEvent(id, { event_type: 'assigned', note: assignee ? `Assigned to ${assignee}` : 'Unassigned', actor: actor || 'admin' });
  }
...
export async function deleteTicket(id) {
  const res = await query(`DELETE FROM tickets WHERE id = $1 RETURNING id`, [id]);
